from fewshot_re_kit.data_loader import get_loader, get_loader_pair, get_loader_unsupervised, get_loader_pair_raw
...
def main():
    ...
    parser.add_argument('--train', default='train_building_v1_fold3', #
            help='train file')
    parser.add_argument('--val', default='val_building_v1_fold3',
            help='val file')
    parser.add_argument('--test', default='test_building_v1_fold3',
            help='test file')
    ...
    parser.add_argument('--raw', action='store_true',
           help='Use raw data to inference')
    ...
    if opt.pair:
        if opt.raw:
            ...
            test_data_loader = get_loader_pair_raw(opt.test, sentence_encoder,
                N=N, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, encoder_name=encoder_name)
        else:
            train_data_loader = get_loader_pair(opt.train, sentence_encoder,
                    N=trainN, K=K, Q=Q, na_rate=opt.na_rate, batch_size=batch_size, encoder_name=encoder_name)
    ...
    acc = framework.eval(model, batch_size, N, K, Q, opt.test_iter, na_rate=opt.na_rate, ckpt=ckpt, pair=opt.pair,raw=opt.raw)
