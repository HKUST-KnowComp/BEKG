...
class FewShotREFramework:
    ...
    def eval(self,
        ...
        print("")
        #pdb.set_trace()
        model.eval()  #What is its usage?
        if raw:
            output = []
            output_path = '/data/pldu/data/rel_building.json'
        if ckpt is None:
        ...
        with torch.no_grad():
            for it in range(eval_iter):
                if pair:
                    if not raw:
                        batch, label = next(eval_dataset) ##TODO
                        if torch.cuda.is_available():
                            for k in batch:
                                batch[k] = batch[k].cuda()
                            label = label.cuda()
                            '''Check the accuracy of each realtion type
                            label_name = label[1]
                            label = torch.tensor(label[0]).cuda()'''

                        logits, pred = model(batch, N, K, Q * N + Q * na_rate)
                    else:
                        batch, label, result = next(eval_dataset)
                        if torch.cuda.is_available():
                            for k in batch:
                                batch[k] = batch[k].cuda()
                            label = label.cuda()
                        logits, pred, score= model(batch, N, K, Q)
                        #pdb.set_trace()
                else:
                ...
                if not raw:
                    '''Check the accuracy of each realtion type
                    flag = (pred == label).type(torch.IntTensor)                    
                    for index in range(len(flag)):
                        result.setdefault(label_name[index], [0,0])
                        result[label_name[index]][flag[index]] += 1 '''
                    #pdb.set_trace()
                    right = model.accuracy(pred, label)
                    iter_right += self.item(right.data)
                    iter_sample += 1
                    ...
                    if float(score) > threshold:
                        iter_sample += 1
                        result['relation'] = result['relation'][int(pred.detach())]
                        result['score'] = float(score)
                        print('Extracting No:{:d}/{:d} relation:'.format(int(iter_sample), it,), result)
                        output.append(result)
                        iter_right = 0
                    else:
                        continue
                    if iter_sample % save_frep == 0 and output:
                        output_path = '/data/pldu/data/rel_building.json'.replace('.json','_' + str(int(iter_sample//save_frep)) + '_s' + str(int(threshold)) + '.json')
                        fw = open(output_path, 'w')
                        print('Saving {:d} to {:d} relations to json file'.format(int(iter_sample-save_frep), int(iter_sample)))
                        json.dump(output, fw)
                        fw.close
                        output = []
                        
                    if it + 1 == eval_iter: 
                        output_path = '/data/pldu/data/rel_building.json'.replace('.json','_' + str(int(iter_sample//save_frep) + 1) + '_s' + str(int(threshold)) +'.json')
                        fw = open(output_path, 'w')
                        print('Saving {:d} to {:d} relations to json file'.format(int(iter_sample-save_frep), int(iter_sample)))
