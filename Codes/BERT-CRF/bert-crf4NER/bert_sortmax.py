...
def train(train_iter, eval_iter, tag2idx, config, bert_model="bert-base-uncased"):
    ...
    model = BertSoftmaxForNer.from_pretrained(bert_model, num_labels = len(tag2idx))
    model.train()
    ...
    for epoch in (train_iterator):
        ...
        for i, batch in enumerate(eval_iter):
            ...
            with torch.torch.no_grad():
                pdb.set_trace()
                tmp_eval_loss, logtis = model(**dev_inputs)
                #tag_seqs = model(**inputs)
                #tmp_eval_loss = model(**dev_inputs)
            val_loss += tmp_eval_loss.item()
            pdb.set_trace()
            preds = np.argmax(logtis.cpu().numpy(), axis=2).tolist()
            tag_seqs = preds[0][1:-1] # [CLS]XXXX[SEP]
            #print(labels.numpy())
