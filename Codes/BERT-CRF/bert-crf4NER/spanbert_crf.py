...
class NER_Dataset(data.Dataset):
    def __init__(self, tag2idx, sentences, labels, tokenizer_path = '', do_lower_case=True):
        ...
        self.labels = labels
        self.tokenizer = AutoTokenizer.from_pretrained("SpanBERT/spanbert-base-cased") #BertTokenizer.from_pretrained(tokenizer_path, do_lower_case=do_lower_case)
...
def generate_training_data(config, bert_tokenizer="SpanBERT/spanbert-base-cased", do_lower_case=True):
...
def generate_test_data(config, tag2idx, bert_tokenizer="SpanBERT/spanbert-base-cased", do_lower_case=True):
...
def train(train_iter, eval_iter, tag2idx, config, bert_model="bert-base-uncased"):
    ...
    pdb.set_trace()
    model = SpanBert_CRF(num_labels=len(tag2idx), hidden_dropout_prob=0.1, hidden_size=768)
    model.train()
...
def load_model(config, do_lower_case=True):
    ...
    unique_labels = list(tag2idx.keys())
    model = SpanBert_CRF.from_pretrained(config.bert_model, num_labels=len(tag2idx))
