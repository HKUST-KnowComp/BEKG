...
def abstract2sent(abstract:dict, target:int):
    sents_all = []
    for idx, title in enumerate(abstract.keys()):
        sents_all.append(nltk.sent_tokenize(abstract[title], language='english'))
        sents_all[idx].insert(0, title)
        if idx+1 == target:
            break
    return sents_all
